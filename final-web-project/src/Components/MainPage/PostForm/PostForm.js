...
const PostForm = () => {
    ...
    // PARA QUE LA PAGINA SE RECARGUE AL CREAR UN NUEVO POST (Se implementa?)
    // const reloadPage = () => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        if (formBody.title === "" || formBody.description === "" || formBody.picture === "") return toast('Llena todos los datos', { type: 'warning' });
    ...
    return (
        <div className="w-4/5 h-auto max-h-72 bg-gray-100 border flex flex-col items-center rounded-lg shadow-lg tablet:w-1/2">
            ...
            <main className={`${active ? '' : 'hidden'} w-96`}>
                <form onSubmit={onSubmitHandler}
                    ...
                    name="title" id="title" className="w-5/6 rounded px-2 py-2 mb-1" minLength="8"/>

                    <input placeholder="Descripción" 
                    name="description" id="description" className="w-5/6 rounded px-2 py-2 mb-1 max-h-24 h-24" minLength="8" />

                    <input placeholder="Imagen (URL)" name="image" id="image" className="w-5/6 rounded px-2 py-2 mb-2" />
